...
class ShareProfileController extends GetxController {
  ...
  getData() async {
    var response = await http.get(
        Uri.parse("$api/AdminInfo/AdminInfo?userid=2"),
  ...
  operation(int scheduleid, int adminid, int paitentid) async {
    if (userID.text.isNotEmpty) {
      ...
      Map<String, dynamic> data = {
        ...
        "closedate": '${now.year}/${now.month}/${now.day}',
      };
      var response = await http.post(Uri.parse("$api/Schedule/ShareFiles"),
...
class ShareProfile extends StatelessWidget {
  ...
  const ShareProfile({required this.title, required this.data, super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder(
        init: ShareProfileController(),
        builder: (controller) {
          return Scaffold(
              appBar: AppBar(
                // back button
